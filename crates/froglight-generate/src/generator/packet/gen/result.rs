...
impl Result {
    ...
    #[must_use]
    pub const fn item_from(kind: Type) -> Self { Self::item_from_attrs(kind, Vec::new()) }

    /// Create a new [`Result::Item`] with the given [`Type`] and
    /// [`Attributes`](Attribute).
    #[must_use]
    pub const fn item_from_attrs(kind: Type, attrs: Vec<Attribute>) -> Self {
        Self::Item { kind, attrs }
    }

    /// Create a new [`Result::Item`] from the given [`String`].
    #[must_use]
    pub fn item_from_str(kind: impl AsRef<str>) -> Self {
...
impl Result {
    ...
    #[must_use]
    pub fn unsupported() -> Self { Self::item_from_str("Unsupported") }
    ...
    /// ```
    #[must_use]
    pub fn map_item(self, fun: impl FnOnce(String) -> String) -> Self {
        match self {
